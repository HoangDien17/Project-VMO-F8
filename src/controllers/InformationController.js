...
let apiUpdateAvatar = (req, res) => {
  avatarUpload(req, res, async (error) => {
    ...
    try {
      let itemUpdate = { avatar: req.file.filename };
      const { statusCode, message, data } = await InforService.apiUpdateInfo(
        req.payload.id,
        itemUpdate
      );
...
let apiUpdateInfor = async (req, res) => {
  const { statusCode, message, data } = await InforService.apiUpdateInfo(
    req.payload.id,
    req.body
  );
...
let apiCreateInfor = async (req, res) => {
  const { statusCode, message, data } = await InforService.apiCreateInfor(
    req.body,
    req.payload.id
  );
  return res.status(statusCode).send({ statusCode, message, data });
...
let apiGetInfor = async (req, res) => {
  const { statusCode, message, data } = await InforService.apiGetInfor(
    req.payload.id
  );
  return res.status(statusCode).send({ statusCode, message, data });
};
